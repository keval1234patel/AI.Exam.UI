...
import { LucideAlertCircle, LucideAngularModule } from 'lucide-angular';
...
@Input({ required: true }) question!: Question;
  ...
  get displayOptions() {
    ...
    if (this.question.options && this.question.options.length > 0) return this.question.options;
    if (this.question.questionType === 'TF') return ['True', 'False'];
    return [];
  }

  calculateStatus() {
    const normalize = (str: string) => str ? str.split(',').map(s => s.trim().toLowerCase()) : [];
  ...
  getOptionClass(opt: string): string {
    ...
    if (isCorrect && isSelected) return 'border-green-500 bg-green-50 text-green-900 shadow-sm';
    if (isCorrect && !isSelected) return 'border-green-300 bg-green-50/50 text-green-800 border-dashed';
    if (!isCorrect && isSelected) return 'border-red-500 bg-red-50 text-red-900 shadow-sm';
